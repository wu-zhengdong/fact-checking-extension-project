# Supplementary Table S2. Item-specific accuracy in Module 1.
...
module1_specs <- list(
  list(
    ...
    var = "statements2_s",
    label = "Girls aged 9-15 should be prioritized for HPV vaccination."
  ),
  list(
    section = "Correct information exposure",
    var = "statements1_s",
    label = paste(
      "The most common high-risk types of human papillomavirus (HPV)",
      "are types 16 and 18."
    )
  ),
  list(
    section = "Correct information exposure",
    var = "statements4_s",
    label = "HPV infection is the equivalent of the cervix getting a cold."
  ),
  list(
    section = "Misinformation exposure",
    var = "statements8_s",
    label = "HPV vaccination may cause infertility."
  ),
  list(
    section = "Misinformation exposure",
    var = "statements9_s",
    label = "HPV vaccination may cause poisoning."
  ),
  list(
    section = "Misinformation exposure",
    var = "statements5_s",
    label = "HPV vaccine should not be given while on antibiotics."
  )
)

module1 <- make_item_table(
  dat,
  module1_specs,
  group_var = "treatment",
  did_var = "did",
...
write.csv(module1, file.path(out_dir, "Supplementary_Table_S2.csv"), row.names = FALSE)
